...
#
# This file is a copy of meta-fsl-arm/classes/image_type_fsl.bbclass
# This copy exists because our barebox needs a image with kernel and not a image with a fat with a kernel
#
...
generate_imx_sdcard () {
	...
	# Create boot partition image
        if [ "${DMO_NO_FAT_FOR_KERNEL}" == "" ]; then
            ...
            dd if=${DEPLOY_DIR_IMAGE}/uImage-${MACHINE}.bin of=${SDCARD} conv=notrunc seek=1 bs=$(expr ${IMAGE_ROOTFS_ALIGNMENT} \* 1024) && sync && sync
        fi
